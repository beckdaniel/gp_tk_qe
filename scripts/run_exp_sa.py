...
def run_experiment(model_type):
    for fold in xrange(1):
        ...
        if DATASET == 'en-es':
            en_buckets = {'NP':1, 'VP': 2, 'S': 3}
            es_buckets = {'grup.nom': 1, 'sn': 2, 'spec': 3}
        elif DATASET == 'fr-en':
            en_buckets = {'NP': 1, 'PP': 2, 'VN': 3}
            es_buckets = {'NP':1, 'VP': 2, 'PP': 3}
        normalize = True

        en_tk = GPy.kern.SymbolAwareSubsetTreeKernel(_lambda=L, _sigma=S, normalize=normalize, active_dims=[0], parallel=True, num_threads=NUM_THREADS, lambda_buckets=en_buckets, sigma_buckets=en_buckets)
        es_tk = GPy.kern.SymbolAwareSubsetTreeKernel(_lambda=L, _sigma=S, normalize=normalize, active_dims=[1], parallel=True, num_threads=NUM_THREADS, lambda_buckets=es_buckets, sigma_buckets=es_buckets)
